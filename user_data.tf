...
frontend_user_data = <<EOF
        ...
        - |-
          cd /home/${var.brand}/public_html/
          su ${var.brand} -s /bin/bash -c "git init -b ${local.environment}"
          su ${var.brand} -s /bin/bash -c "git remote add origin codecommit::${data.aws_region.current.name}://${aws_codecommit_repository.magento.repository_name}"
          su ${var.brand} -s /bin/bash -c "git fetch origin ${local.environment}"
          su ${var.brand} -s /bin/bash -c "git reset origin/${local.environment} --hard"
        - |-
          sed -i "s/INSTANCE_LOCAL_IP/$${INSTANCE_LOCAL_IP}/" /etc/nginx/sites-available/magento.conf
          systemctl restart nginx php${var.php_version}-fpm
EOF
}
