...
resource "aws_codepipeline" "this" {
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        S3Bucket             = aws_s3_bucket.this["system"].bucket
        S3ObjectKey          = "releases/placeholder.zip"
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ...
        PrimarySource = "build_source_output"
      }
    }
  }
  stage {
    ...
    action {
      ...
      configuration = {
        ...
        CustomData      = "Approve codepipeline [#{codepipeline.PipelineExecutionId}] Deploy action for ${local.project} [#{SourceVariables.VersionId}]"
      }
    }
    dynamic "action" {
